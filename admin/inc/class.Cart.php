...
class Cart {
	...
	*/
	...
	public function __construct($currency = "", $dph = 20){
	...
	public function calculate(){
		...
		foreach ($_SESSION["cart"] as $item){
			...
			$this->totalQuantity += $d[0];
                        $this->totalPriceDph += round(($d[1] * $this->dph),2) * $d[0] ;
		}
	}
	...
	* @return full price of products
	*/
	public function getTotalPriceWithCurrencyAndDPH(){
		return number_format($this->totalPriceDph, 2, ",", " ")." ".$this->currency;
	}
	
        
        public function getDph(){
            return number_format(round($this->totalPriceDph - ($this->totalPriceDph / $this->dph), 2), 2, ",", " ")." ".$this->currency;
        }
        
        public function getPercentageDph(){
            return ($this->dph - 1) * 100;
        }

        
        public function getDeliveryPriceDph(){
            if(isset($_SESSION['dp'])){
		$d = explode("-", $_SESSION['dp']);
                return number_format($d[2], 2, ",", " ")." ".$this->currency;
            }
            throw new Exception("Delivery is not set.");
        }
        ...
        public function getTotalOrderPrice(){
            if(isset($_SESSION['dp'])){
		$d = explode("-", $_SESSION['dp']);
                return number_format($d[2] + $this->totalPriceDph, 2, ",", " ")." ".$this->currency;
            }
            throw new Exception("Delivery is not set.");
        }
